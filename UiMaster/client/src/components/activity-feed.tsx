...
export default function ActivityFeed() {
  ...
  if (leaveData) {
    leaveData.slice(0, 3).forEach(leave => {
      const leaveStatus = leave.status.charAt(0).toUpperCase() + leave.status.slice(1);
      
      activities.push({
        id: `leave-${leave.id}`,
        type: 'leave',
        title: `Your leave request has been ${leave.status === 'pending' ? 'submitted' : leave.status}`,
        timestamp: leave.appliedAt,
        icon: <Calendar className="h-4 w-4" />,
        iconBg: leave.status === 'approved' ? 'bg-green-100' : 
               leave.status === 'rejected' ? 'bg-red-100' : 'bg-yellow-100',
        iconColor: leave.status === 'approved' ? 'text-green-600' : 
          ...
          <ul className="divide-y divide-gray-200">
            {activities.slice(0, 5).map((activity) => (
              <li key={activity.id} className="py-3 flex items-start">
                <span className={`flex-shrink-0 h-8 w-8 rounded-full ${activity.iconBg} flex items-center justify-center mr-3`}>
